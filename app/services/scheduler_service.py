...
def calculate_all_projects_automatically():
    ...
    for dates that have APPROVED work logs but haven't been calculated yet.
    ...
    try:
        ...
        for project in active_projects:
            ...
            for calc_date in dates_to_process:
                try:
                    # Check if there are any APPROVED logs for this project and date
                    # For today's date, also check PENDING logs (they might be auto-approved)
                    if calc_date == today:
                        # For today, check both APPROVED and PENDING (in case auto-approval hasn't run)
                        has_logs = db.query(TimeHistory).filter(
                            TimeHistory.project_id == project.id,
                            TimeHistory.sheet_date == calc_date,
                            TimeHistory.status.in_(["APPROVED", "PENDING"]),
                            TimeHistory.clock_out_at.isnot(None)  # Only completed sessions
                        ).first()
                    else:
                        # For past dates, only process APPROVED logs
                        has_logs = db.query(TimeHistory).filter(
                            TimeHistory.project_id == project.id,
                            TimeHistory.sheet_date == calc_date,
                            TimeHistory.status == "APPROVED"
                    ...
                    # For today's PENDING entries, auto-approve them first
                    if calc_date == today:
                        pending_entries = db.query(TimeHistory).filter(
                            TimeHistory.project_id == project.id,
                            TimeHistory.sheet_date == calc_date,
                            TimeHistory.status == "PENDING",
                            TimeHistory.clock_out_at.isnot(None)  # Only completed sessions
                        ...
                        for entry in pending_entries:
                            ...
                            # Calculate minutes_worked if not set
